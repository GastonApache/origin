-- client.lua : FiveM client helper to control the circular progress NUI
-- Lit ProgressConfig (défini par config.lua) et l'envoie au NUI
-- Ajoute StartProgressDuration/StopProgressDuration pour animer la progression sur une durée donnée.
...
-- safe default if ProgressConfig not set
local function getConfigOrDefault()
  local c = ProgressConfig or {}
  ...
  return {
    ...
    bgColor = c.bgColor or 'rgba(200,200,200,0.6)',
    ...
    width = toPx(c.width) or '200px',
    height = toPx(c.height) or '200px',
    showPercent = (c.showPercent == nil) and true or c.showPercent,
    fontFamily = c.fontFamily or 'sans-serif',
    fontWeight = c.fontWeight or 'normal',
    fontSizeScale = tonumber(c.fontSizeScale) or 0.8,
    lineWidthScale = tonumber(c.lineWidthScale) or 0.18,
    zIndex = tonumber(c.zIndex) or 5000,
    fontFile = c.fontFile -- raw path relative to html/, e.g. 'fonts/MyFont.ttf'
  }
end
...
local function StartAuto()
  ...
  autoThread = CreateThread(function()
    while autoRunning do
      Wait(800)
      current = (current >= 100) and 0 or (current + 10)
...
local function StartProgressDuration(durationSeconds, hideOnComplete)
  ...
  durationThread = CreateThread(function()
    ...
    local startedValue = 0
    sendProgress(startedValue)
    sendUI(true)
...
-- Exports (so other resources can call these)
...
-- Register a simple chat command to test quickly
RegisterCommand('cprogress', function(source, args, raw)
  local cmd = args[1]
  if not cmd then
    print("Usage: /cprogress start|stop|set <0-100>|show|hide|duration <seconds> [hide]")
    return
  end
  if cmd == 'start' then
    sendUI(true)
...
-- Au démarrage, on envoie la config NUI (depuis config.lua ou valeurs par défaut)
CreateThread(function()
  Wait(500)
  local cfg = getConfigOrDefault()
  -- Envoie la config entière, y compris fontFile si fourni
  SendNUIMessage({
    type = 'config',
    config = cfg
  })
  ...
  -- Si configuré pour auto-start via config.lua
  if ProgressConfig and ProgressConfig.autoStartOnResource then
    sendUI(true)
    StartAuto()
  end
end)